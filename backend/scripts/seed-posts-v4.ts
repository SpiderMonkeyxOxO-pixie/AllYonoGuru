/**
 * Seeds seed-data-posts-v4.json into Strapi as DRAFTS.
 * Usage (from backend/):
 *   STRAPI_URL=http://localhost:1338 STRAPI_SEED_TOKEN=<token> npx ts-node scripts/seed-posts-v4.ts
 */
import fs from "fs";
...
const data = JSON.parse(
  fs.readFileSync(path.join(__dirname, "..", "seed-data-posts-v4.json"), "utf-8")
);
